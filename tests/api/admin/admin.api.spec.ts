import { expect, apiFixtures } from "../../../src/fixtures/api-fixtures.js";
...
apiFixtures.describe('Admin API tests', () => {

    const password = process.env.TEST_PASSWORD!;

    apiFixtures('POST user - returns 201 when user is created', { tag: ['@api', '@smoke', '@admin', '@p0'] }, async ({ adminService, cleanupSingleTestUser }) => {
    ...
    apiFixtures('POST user - returns 422 when email already exists', { tag: ['@api', '@regression', '@admin', '@validation', '@p1'] }, async ({ adminService, cleanupSingleTestUser, setupSingleTestUser }) => {
    ...
    apiFixtures('POST user - returns 422 when username already exists', { tag: ['@api', '@regression', '@admin', '@validation', '@p1'] }, async ({ adminService, cleanupSingleTestUser, setupSingleTestUser }) => {
        const user = await setupSingleTestUser;
    ...
    apiFixtures('DELETE user - returns 204 when user exists', { tag: ['@api', '@regression', '@admin', '@validation', '@p0'] }, async ({ adminService, setupSingleTestUser }) => {
        const user = await setupSingleTestUser;
        const response = await adminService.deleteUser(user.username);
        expect(response.status()).toBe(204);
    });

    apiFixtures('DELETE user- returns 404 when user does not exist', { tag: ['@api', '@regression', '@admin', '@validation', '@p1'] }, async ({ adminService }) => {
