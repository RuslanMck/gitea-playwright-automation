import { test, expect } from "../../src/fixtures/page-fixtures.js";


test.describe('Public Home Page test', () => {
    ...
    test('Homepage - Registration page is opened after clicking correspondent button', { tag: ['@ui', '@smoke', '@homepage', '@p0'] }, async ({ publicHomePage, registrationPage }) => {
        await publicHomePage.open();
        await publicHomePage.clickRegisterButton();
        await expect(registrationPage.registrationForm).toBeVisible();
    });

    test('Homepage - Sign In page is opened after clicking correspondent button', { tag: ['@ui', '@smoke', '@homepage', '@p0'] }, async ({ publicHomePage, signInPage }) => {
        await publicHomePage.open();
